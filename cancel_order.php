...
try {
    ...
    if (!csrfValidate($csrf, 'cancel_order')) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'Security token validation failed.']);
        exit;
    }

    $result = cancelPendingOrder($pdo, $orderId, isset($_SESSION['admin']) ? (int) $_SESSION['admin'] : null);

    echo json_encode([
        'success' => true,
        'message' => $result['already_cancelled'] ? 'Order was already cancelled.' : 'Order cancelled successfully.',
        'result' => $result,
    ]);
} catch (Throwable $e) {
