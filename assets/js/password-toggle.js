(function () {
    ...
    function shouldBlockSpaces(input) {
        if (!input) return false;
        if (input.dataset.allowSpaces === '1') return false;
        // Account passwords: block spaces by default. SMTP / API keys may allow spaces.
        if (input.dataset.noSpaces === '1') return true;
        const name = String(input.name || input.id || '').toLowerCase();
        if (name.includes('smtp') || name.includes('api_key') || name.includes('mail_')) {
            return false;
        }
        return input.type === 'password' || input.dataset.passwordToggle === '1';
    }

    function stripSpaces(input) {
        if (!shouldBlockSpaces(input)) return;
        const next = String(input.value || '').replace(/\s+/g, '');
        if (next !== input.value) {
            input.value = next;
        }
    }
    ...
    function bindNoSpaces(input) {
        if (!input || input.dataset.spaceFilter === '1' || !shouldBlockSpaces(input)) {
            return;
        }
        input.dataset.spaceFilter = '1';
    ...
    function init() {
        ...
        document.querySelectorAll('input[data-no-spaces="1"]').forEach(bindNoSpaces);
    }
